...
// Owner-approved unofficial GMP providers, enabled by default in production.
// An explicit GMP_SOURCE_ALLOWLIST env var overrides this for toggling.
const DEFAULT_GMP_ALLOWLIST = "ipoji,investorgain,ipotrack";
...
export function isGmpSourceEnabled(key: string, allowlist = process.env.GMP_SOURCE_ALLOWLIST ?? DEFAULT_GMP_ALLOWLIST): boolean {
...
export function enabledGmpAdapters(adapters: GmpAdapter[], allowlist = process.env.GMP_SOURCE_ALLOWLIST ?? DEFAULT_GMP_ALLOWLIST): GmpAdapter[] {
